...
class Solver:
                ...
                for firstNodeIndex in range(1, len(rt1.sequenceOfNodes) - 1):
                    ...
                    for secondNodeIndex in range(startOfSecondNodeIndex, len(rt2.sequenceOfNodes) - 1):
                        ...
                        if rt1 != rt2:
                            ...
                            cost1, d1 = self.calculate_route_details(rt1.sequenceOfNodes, 8)
                            cost2, d2 = self.calculate_route_details(rt2.sequenceOfNodes, 8)
                ...
                for nodeInd1 in range(0, len(rt1.sequenceOfNodes) - 1):
                    ...
                    for nodeInd2 in range(start2, len(rt2.sequenceOfNodes) - 1):
                        ...
                        if rt1 == rt2:
                            ...
                            cost_change_rt1 += self.distanceMatrix[A.ID][K.ID] * (d_after_origin_node_rt1 + 8) + \
                                                self.distanceMatrix[B.ID][L.ID] * (d_after_target_node_rt1 + 8) - \
                                                self.distanceMatrix[A.ID][B.ID] * (d_after_origin_node_rt1 + 8) - \
                                                self.distanceMatrix[K.ID][L.ID] * (d_after_target_node_rt1 + 8)
                            ...
                            load_of_rt1s_segment = sum(node.demand for node in relocated_segment1) + 8
                            load_of_rt2s_segment = sum(node.demand for node in relocated_segment2) + 8
                ...
                for originNodeIndex in range(1, len(rt1.sequenceOfNodes) - 1):
                    for targetNodeIndex in range(0, len(rt2.sequenceOfNodes) - 1):
                        ...
                        if rt1 != rt2:
                            ...
                            cost_change_rt1 = self.distanceMatrix[A.ID][C.ID] * (d_after_origin_node_rt1 + 8) - \
                                self.distanceMatrix[A.ID][B.ID] * (d_after_origin_node_rt1 + B.demand + 8) - \
                                self.distanceMatrix[B.ID][C.ID] * (d_after_origin_node_rt1 + 8)
                            ...
                            cost_change_rt2 = self.distanceMatrix[F.ID][B.ID] * (d_after_target_node_rt2 + B.demand + 8) + \
                                self.distanceMatrix[B.ID][G.ID] * (d_after_target_node_rt2 + 8) - \
                                self.distanceMatrix[F.ID][G.ID] * (d_after_target_node_rt2 + 8)
                            ...
                            if originNodeIndex < targetNodeIndex:
                                ...
                                cost_change_rt1 = self.distanceMatrix[A.ID][C.ID] * (d_after_origin_node_rt1 + B.demand + 8) - \
                                    self.distanceMatrix[A.ID][B.ID] * (d_after_origin_node_rt1 + B.demand + 8) - \
                                    self.distanceMatrix[B.ID][C.ID] * (d_after_origin_node_rt1 + 8) + \
                                    self.distanceMatrix[F.ID][B.ID] * (d_after_target_node_rt2 + B.demand + 8) + \
                                    self.distanceMatrix[B.ID][G.ID] * (d_after_target_node_rt2 + 8) - \
                                    self.distanceMatrix[F.ID][G.ID] * (d_after_target_node_rt2 + 8)
                            else:
                                ...
                                cost_change_rt1 = self.distanceMatrix[A.ID][C.ID] * (d_after_origin_node_rt1 + 8) - \
                                    self.distanceMatrix[A.ID][B.ID] * (d_after_origin_node_rt1 + B.demand + 8) - \
                                    self.distanceMatrix[B.ID][C.ID] * (d_after_origin_node_rt1 + 8) + \
                                    self.distanceMatrix[F.ID][B.ID] * (d_after_target_node_rt2 + 8) + \
                                    self.distanceMatrix[B.ID][G.ID] * (d_after_target_node_rt2 - B.demand + 8) - \
                                    self.distanceMatrix[F.ID][G.ID] * (d_after_target_node_rt2 + 8)
    ...
    def TestSolution(self):
        ...
        for r in range(0, len(self.sol.routes)):
            ...
            rtCost, rtLoad = self.calculate_route_details(rt.sequenceOfNodes, 8)  # 8 is the empty_vehicle_weight

...
class Solver:
    ...
    def Clarke_n_Wright(self):
        self.sol = self.create_initial_routes() # Step 1
        savings: list = self.calculate_savings() # Step 2
        savings.sort(key=lambda s: s.score, reverse=True) # Bigger score first

        for i in range(0, len(savings)): # 300 chose 2 savings
            sav = savings[i]
            n1 = sav.n1
            n2 = sav.n2
            rt1 = n1.route
    ...
    def create_initial_routes(self):
        ...
        for i in range(0, len(self.customers)):
            ...
            rt.sequenceOfNodes.insert(1, n) # [dp, n, dp]
            rt.load = n.demand
    ...
    def calculate_savings(self):
        ...
        for i in range(0, len(self.customers)):
            ...
            for j in range(i + 1, len(self.customers)):
                ...
                score = self.distanceMatrix[n1.ID][self.depot.ID] + self.distanceMatrix[self.depot.ID][n2.ID] - self.distanceMatrix[n1.ID][n2.ID]
